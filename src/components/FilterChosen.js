...
const FilterChosen = ({ name }) => {
	...
	useEffect(() => {
		...
		setEndPages(10);
		name.length > 1
			? name.forEach(async (item) => await getPokemon(item.pokemon.name))
			: name.forEach(async (element) => await getPokemon(element));
	}, [name]);

	useEffect(() => {
		const allPagesNeeded = Array.from(
			Array(Math.ceil(info.length / 4)),
			(x, index) => index + 1
		);
		...
		if (selectedPage > moveNum) {
			...
			if (endPages === allPages.length) {
				...
				if (selectedPage - pageNumber > allPages.length - endPages) {
					setStartPages(
						(startPages) => startPages + (allPages.length - selectedPage)
					);
					setEndPages(
						(endPages) => endPages + (allPages.length - selectedPage)
					);
			...
			if (startPages === 0) {
				...
				if (pageNumber - selectedPage > startPages - 0) {
					...
					setStartPages(
						(startPages) => startPages - (pageNumber - selectedPage)
					);
		...
		<>
			<div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-5 mb-7">
				{info.length > 0 ? (
					info
						.slice(start, end)
						.map((element) => <PokeCard key={element.id} info={element} />)
				) : (
					<p className="text-center w-full col-span-4 underline">
						No pokemon matches your selection, please try again!
					</p>
				)}
			</div>
			<div className="flex mt-3 items-center justify-center">
				<div className="flex justify-start">
					{pageNumber > 1 ? (
						<button onClick={handlePreviousClick} className="mr-2">
							<svg
								className="w-6"
				...
				{allPages &&
					allPages.slice(startPages, endPages).map((item, index) => {
						return item === pageNumber ? (
							<button
								onClick={() => handleClick(item)}
								key={index}
								className="mb-2 bg-black text-white border-2 border-black rounded-full h-10 w-10 flex items-center justify-center mr-2"
							>
								{item}
							</button>
						) : (
							<button
								onClick={() => handleClick(item)}
								key={index}
								className="mb-2 bg-white text-black border-2 border-black rounded-full h-10 w-10 flex items-center justify-center mr-2"
							>
								{item}
							</button>
						);
					})}
				{pageNumber !== allPages.length && info.length > 0 ? (
