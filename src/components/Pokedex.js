		...
		<div className="lg:grid lg:grid-cols-2 lg:gap-2">
					...
					<div className="flex bg-red-500 rounded-xl mb-2 lg:w-full lg:h-auto lg:flex-col lg:items-center lg:text-center">
						...
						<div className="flex flex-col p-5 justify-center">
							<p className="text-gray-700 font-bold">
								#{ubication.props.info.order}
							</p>
							<h3 className="text-white font-bold capitalize text-3xl mb-1">
								{ubication.props.info.name}
							</h3>
							<div className="flex lg:justify-center">
								{ubication.props.info.types.map((type, index) => (
									<span
										key={index}
										className="bg-yellow-600 rounded px-2 py-1 capitalize text-white mr-2"
									>
										{type.type.name}
									</span>
								))}
						...
						<div className="flex w-full lg:flex-col">
							<div className="bg-yellow-200 p-3 rounded-xl mr-2 px-5 text-center w-2/4 h-full lg:w-full lg:mb-2">
								...
								<p className="text-gray-800">
									{(ubication.props.info.height / 10).toFixed(1)} m
								</p>
								<p className="font-bold">Weight:</p>
								<p className="text-gray-800">
									{(ubication.props.info.weight / 10).toFixed(1)} kg
								</p>
							</div>
							<div className="bg-indigo-300 p-3 rounded-xl px-5 w-2/4 mr-2 lg:w-full lg:mb-2">
								<div className="flex justify-center">
									<p className="font-bold mr-1">Hp:</p>
									<p className="text-gray-800">{ubication.props.hp}</p>
								</div>
								<div className="flex justify-center">
									<p className="font-bold mr-1">Attack:</p>
									<p className="text-gray-800">{ubication.props.attack}</p>
								</div>
								<div className="flex justify-center">
									<p className="font-bold mr-1">Defense:</p>
									<p className="text-gray-800">{ubication.props.defense}</p>
								</div>
								<div className="flex justify-center">
									<p className="font-bold mr-1">Speed:</p>
									<p className="text-gray-800">{ubication.props.speed}</p>
								</div>
							</div>
			...
			<div className="flex flex-col bg-gray-400 p-5 rounded-xl lg:w-full">
				...
				<div className="grid grid-cols-3 gap-2 md:grid-cols-4 lg:grid-cols-5">
					{ubication.props.info.moves.length > 0 ? (
						...
						<p>
							Apparently, <b>{ubication.props.info.name}</b> doesn't have any
							moves
						</p>
					)}
				</div>
			...
			<div className="flex flex mt-3 justify-between lg:col-span-2">
				<div className="bg-black text-white font-bold rounded-lg p-2 w-20">
					<Link to="/pokedex" className="flex flex-col items-center">
						<svg
							className="w-6"
				...
				<div className="bg-black text-white font-bold rounded-lg p-2 w-20">
					<Link
						to={{
							pathname: `/pokedex/pokemon/${ubication.props.info.order}/encounters`,
							encounters: ubication.props.info.location_area_encounters,
							name: ubication.props.info.name,
						}}
